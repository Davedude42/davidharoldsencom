...
<ProgramInfo>
	<h4 class="!m-0"><span class="bg-blue-200">Light Blue</span></h4>
...
<div class:hidden={!pgnPopupOpen} class="fixed z-40 inset-0 flex flex-col items-center justify-center bg-black/40">
	<div style="width: 500px; height: 400px; max-width: 90%;" class="border-comical flex flex-col gap-4 p-4 bg-blue-300">
		<div class="flex flex-row justify-between">
			<OutlandishButton elementStyle="width: 150px; padding: 4px;" on:click={copyPGN}>
				Copy
			</OutlandishButton>
			<button class="border-comical p-1 bg-red-500 hover:bg-red-400 active:bg-red-300 font-semibold" style="width: 40px; height: 40px;" on:click={() => pgnPopupOpen = false}>
...
<div class="flex-grow self-stretch flex flex-col md:flex-row justify-evenly md:max-h-screen pt-nav bg-neutral-700 overflow-hidden">
	{#if gameStarted}
		<div class="flex-grow lg:flex-grow-0 flex-shrink flex flex-row items-center justify-center max-w-full max-h-full px-8 overflow-hidden" bind:this={wrapper}>
			<canvas class="box-content border-comical select-none" bind:this={canvas} on:mousedown={mouseDown}></canvas>
		</div>
	{/if}
	<div class="lightblue-grid self-center md:self-stretch">
		<div class="flex flex-row justify-between" style="line-height: 1.4;">
			...
			{#if gameStarted}
				{#if showComputerStuff}
					<div class="lightblue-message items-end">
						<div class="font-bold">{ (board.metaData.time / 1000)?.toFixed(2) } seconds</div>
						<div>{ board.metaData.nodes } nodes</div>
						<div>{ (board.metaData.time / board.metaData.nodes)?.toFixed(2) }ms per node</div>
					</div>
				{:else if !loading}
		...
		{#if gameStarted} 
			<div class="grid grid-cols-2 overflow-hidden" style="min-height: 300px;">
				<div class="flex flex-col gap-4 p-4">
					{#if gameOver} 
						<div class="p-2 font-IBMPlexMono bg-blue-400 font-semibold text-center text-xl">
							{#if draw} 
								Draw!
							{:else}
				...
				<div class="grid grid-cols-2 gap-y-2 p-2 content-start items-start justify-items-start font-IBMPlexMono overflow-y-scroll" bind:this={notationScroll}>
					{#each notation as note}
						<div class="px-1 bg-orange-200 font-bold">
							{ note }
						</div>
					{/each}
				</div>
